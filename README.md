...
- **实时架构（/architecture）**：5s 自动刷新，按 云 Data Center / 边 hospital / 端 clinic 三层展示组件健康（/health）、节点负载（/cluster/summary）、任务总线（/tasks/stats）与四类任务流程。
  ...
  - 轨迹上方新增 **`当前集群架构总览 · LIVE CLUSTER OVERVIEW`**：4 节点卡片（控制面/边×2/云）实时显示角色、Ready、CPU/内存数值与条、承载 Pod 图标与数量、节点间关系连线。
...
| 镜像 | tag | 说明 |
|---|---|---|
| `k8s-repo/inference-scheduler` | v3.0.8 | 控制面(迁 node3) + /cluster/* + React 前端 + 测试数据 |
