...
## 测试与性能对比站点（benchmark-site v1.0）

**独立部署、不依赖 scheduler 存活**（这是能做掉线演练的前提——平台前端 `/app` 由 scheduler Pod 托管）。

| 入口 | 地址 | 说明 |
|---|---|---|
| 对比站点 | `http://<任一节点IP>:30082/` | 测试下发 / 实时状态 / 结果明细 / 性能对比 / 实验配置 |
| 站点 API | `/api/health`、`/api/runs`、`/api/compare`、`/api/compare/matrix`、`/api/export.csv`、`/api/export.json` … | 见设计方案 §6.1 |
