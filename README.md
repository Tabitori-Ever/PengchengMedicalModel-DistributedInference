...
```bash
bash build.sh        # 先 npm 构建 frontend/dist，再构建并推送 inference-scheduler/hospital/clinic:v2.0
```
...
| 镜像 | tag | 说明 |
|---|---|---|
| `k8s-repo/inference-scheduler` | v2.0 | 控制面 + /cluster API + React 前端(dist) |
| `k8s-repo/hospital` | v2.0 | worker + part1 合并，~1.8GB |
| `k8s-repo/clinic` | v2.0 | 内存监控，~220MB |
