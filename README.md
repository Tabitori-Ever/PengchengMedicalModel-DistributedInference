...
- **实时架构（/architecture）**：5s 自动刷新，按 云 Data Center / 边 hospital / 端 clinic 三层展示组件健康（/health）、节点负载（/cluster/summary）、任务总线（/tasks/stats）与四类任务流程。
  ...
  - 轨迹上方为 **`当前集群架构总览 · LIVE CLUSTER OVERVIEW（逻辑架构视图 · LOGICAL VIEW）`**：按 `01 CLOUD 云 Data Center` / `02 EDGE 边 hospital` / `03 TERMINAL 端 clinic` / `04 OPS 运维` 分组面板展示逻辑实体（无边框图标 + 能力说明 + `@node` 标注 + 实时健康/负载），并绘制关系连线：边↔云「协同诊断/协同计算/数据同步/日常任务」、端↔云「协同计算/数据同步/日常任务」、clinic⇢hospital 点线「医疗推理转诊」、运维⇢云 点线「Prometheus/metrics-server 观测」、云内 scheduler—medical-server/dc-services/redis。
...
| 镜像 | tag | 说明 |
|---|---|---|
| `k8s-repo/inference-scheduler` | v3.0.9 | 控制面(迁 node3) + /cluster/* + React 前端 + 测试数据 |
