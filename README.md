...
- **实时架构（/architecture）**：5s 自动刷新，按 云 Data Center / 边 hospital / 端 clinic 三层展示组件健康（/health）、节点负载（/cluster/summary）、任务总线（/tasks/stats）与四类任务流程。
  ...
  - 轨迹上方为 **`当前集群架构总览`（逻辑架构视图）**：按 `云数据中心` / `医院 · 边` / `诊所 · 端` / `运维` 分组面板展示逻辑实体（无边框图标 + 简短中文说明 + 实时健康/负载），连线精简为：边⇄云「协同诊断 / 计算 / 同步 / 日常」、端⇄云「协同计算 / 同步 / 日常」、诊所→医院点线「转诊」、运维→云点线「指标观测」、云内调度—医疗推理/患者库/队列连接。
...
| 镜像 | tag | 说明 |
|---|---|---|
| `k8s-repo/inference-scheduler` | v3.0.10 | 控制面(迁 node3) + /cluster/* + React 前端 + 测试数据 |
