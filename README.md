...
| 用户调用平台 | `/app/user/` | 类 ChatGPT 对话式调用：点任务类型 → 该气泡内即时出现参数表单 → 助手气泡显示进度 → 完成后给结论；左侧历史任务列表 |
| 管理平台 | `/app/admin/` | `平台总览` / `可视化` / `集群负载` / `综合测试` / `集群编排` |
| 示例数据集 | `/app/datasets/` | `index.json` + 四类任务的示例数据文件，供用户平台「选择数据集」直接选用 |

- **用户平台（v3.1.2）**：点击任务类型**立即**在当前气泡内展开表单（草稿消息模型：再次点击其它类型就地改写，不新增气泡；提交后同一气泡带上任务号并只追加助手回复）。参数已极简——诊断仅选发起方（固定 **医院 1 / clinic-1**，自动转诊）与患者；计算选 **小/中/大** 规模预设；通信选 **普通/省流/高速** 方式；日常仅填任务数；其余收进「高级参数」（默认收起），**全站不再出现优先级**。点「选择数据集」按当前任务类型列出 `/app/datasets/` 中的示例（名称/说明/大小），选中即按文件内容预填参数，也可「从本机选择文件」（JSON 预填，其它格式仅记录文件名与大小，文件体不上传）。结果**只给结论**：诊断给 bpCR 概率大字与 pCR/non-pCR 判定、计算给分区数与数据量、通信给拉取/上传与备份与否、日常给完成数；运行中仅进度条，**不显示时延、阶段明细、执行节点与指标表**；页面内无跨平台跳转与说明性文案。
- **平台总览**：三级架构与四类任务说明、节点/实体/实例/任务/组件健康 KPI、任务类型分布与各功能入口。
- **可视化**：逻辑架构总览（数据中心 / 医疗中心 / 医院 / 运维四组实体 + 关系连线，**不显示负载**）→ 任务选择（前缀优先下拉）→ 信息条 → **多任务并发·调度态势**（**始终展示最近任务**，最多 12 条；窗口起点取「10 分钟前」与最早任务起点之较早者，故历史任务同样成行并在标题旁标注「最近任务（无进行中任务）」；颜色区分任务类型、形状区分来源层级、运行中实心脉冲、失败锈红，右侧调度队列按优先级排序并标注并发上限 4）→ 任务执行轨迹（按执行时间顺序高亮：已执行灰 / 当前橙 / 待执行淡灰虚 / 无关最淡；并行步骤 `∥N` 同时点亮；手动 `⏮ 上一步 下一步 ▶` 与自动循环演示 2s/步并带过渡）→ 执行时序 → 图例。
...
- 示例数据集为纯静态文件，随前端构建产物发布（`frontend/public/datasets/` → `/app/datasets/`），新增数据只需放入该目录并登记到 `index.json`。

## 快速开始

```bash
# 构建并推送（hospital/clinic/dc 为 v3.0，scheduler 为 v3.1.2）
bash build.sh
