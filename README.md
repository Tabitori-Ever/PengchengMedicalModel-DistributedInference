...
- **实时架构（/architecture）**：5s 自动刷新，按 云 Data Center / 边 hospital / 端 clinic 三层展示组件健康（/health）、节点负载（/cluster/summary）、任务总线（/tasks/stats）与四类任务流程。
  - 顶部新增**任务执行轨迹地图（TASK TRAJECTORY MAP）**：上中下=云/边/端三层，可按 诊断/计算/通信/日常 切换轨迹模板；依据真实任务数据按**执行时间顺序**逐步高亮节点与连线（已执行=黄铜 / 当前=橙色脉冲+流动光点 / 待执行与无关=分层灰化），地图下方提供**执行时序时间轴**（① 提交 · 12ms ② 调度 …，可点击跳步、▶ 按序播放 / ⏸ 暂停；完成任务自动按序播放一次，运行中按 stage 实时跟随）；默认自动跟随该类型最新任务，可手动选择最近任务查看。
...
| 镜像 | tag | 说明 |
|---|---|---|
| `k8s-repo/inference-scheduler` | v3.0.6 | 控制面(迁 node3) + /cluster/* + React 前端 + 测试数据 |
