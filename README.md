...
| 用户调用平台 | `/app/user/` | 类 ChatGPT 对话式调用：四类任务卡片 → 表单气泡（可选上传 JSON 预填/备注）→ 助手气泡实时显示阶段·进度·执行者 → 完成后内联结果；左侧历史任务列表 |
| 管理平台 | `/app/admin/` | `可视化` / `集群负载` / `综合测试` / `集群编排` |

- **可视化**：逻辑架构总览（数据中心 / 医疗中心 / 医院 / 运维四组实体 + 关系连线，**不显示负载**）→ 任务执行轨迹（按执行时间顺序高亮：已执行灰 / 当前橙 / 待执行淡灰虚 / 无关最淡；并行步骤 `∥N` 同时点亮；手动 `⏮ 上一步 下一步 ▶` 与自动循环演示 2s/步并带过渡；任务选择为前缀优先下拉）→ **多任务并发·调度态势**（同一时间窗内多任务并发条形，颜色区分任务类型、形状区分来源层级、运行中实心脉冲、失败锈红，右侧调度队列按优先级排序并标注并发上限）→ 任务总线。
...
- 前端 GET 请求失败自动重试一次；后端 v3.0.12 起 `/cluster/status`、`/cluster/summary` 服务端缓存 4s、新增轻量 `/tasks/recent`、`/app/assets/*` 长缓存；轮询 10s 且标签页隐藏时暂停。
## 快速开始
